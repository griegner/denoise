...
def get_args(strategies):
    ...
    parser.add_argument(
        "--report_only", action="store_true", help="run group-level summary only"
    )
...
class Data:
    ...
    def get_masks(self):
        masks = self.fmriprep.glob(
            "**/sub-*_task-n*_space-MNI152NLin6Asym_res-2_desc-brain_mask.nii.gz"
        )
        return sorted([str(mask) for mask in masks])

    def get_preprocs(self):
        preprocs = self.fmriprep.glob(
            "**/sub-*_task-n*_space-MNI152NLin6Asym_res-2_desc-preproc_bold.nii.gz"
        )
    ...
    def get_motion_dfs(self):
        dfs = sorted(
            self.fmriprep.glob("**/sub-*_task-n*_desc-confounds_timeseries.tsv")
        )
        return [
            pd.read_csv(
                df, sep="\t", usecols=["framewise_displacement", "std_dvars", "rmsd"]
            )
            for df in dfs
        ]
...
def get_correlations(timeseries, path_none, path_vector, strategy):
    correlation_vector = ConnectivityMeasure(
    ...
    np.fill_diagonal(matrix[0], np.nan)
    np.fill_diagonal(matrix[1], np.nan)
...
def run_summary(
    vector, matrix, sub, task, motion_df, fd_thresh, dvars_thresh, preproc, mask, paths
):
    fig, axs = plt.subplots(
        6, figsize=(4, 9), gridspec_kw={"height_ratios": [2, 10, 1, 1, 2, 2]}
    )
    for ax in axs:
        ax.axis("off")
        ax.margins(0, 0.02)
...
def group_summary(derivatives, strategy, atlas):
    ...
    fig, axs = plt.subplots(
        ncols=4, figsize=(20, 2), gridspec_kw={"width_ratios": [1, 2, 2, 2]}
    )
    for ax in axs:
        ax.axis("off")
        ax.margins(0, 0)
...
def main():
    ...
    if not args.report_only:
        ...
        for mask, preproc, confound, motion_df in zip(
            data.masks, data.preprocs, data.confounds, data.motion_dfs
        ):

            file_entities = parse_file_entities(preproc)
            sub, task = file_entities["subject"], file_entities["task"]
            print(f"> sub-{sub} task-{task}")
            path_plot, path_none, path_vector, path_qc = build_path(
                derivatives, sub, task, args.strategy
            )
            motion_df.to_csv(path_qc, sep="\t", index=False)

            mask, preproc_noise = image.load_img(mask), image.load_img(preproc)
            assert (
                mask.shape == preproc_noise.shape[:-1]
            ), "mask and preproc images have different shapes"
            preproc_denoise = image.clean_img(
                preproc,
                detrend=False,
                standardize=False,
                confounds=confound,
            ...
            timeseries = get_timeseries(
                preproc, atlas, mask, args.smooth_fwhm, derivatives
            )

            vector, matrix = get_correlations(
                timeseries, path_none, path_vector, args.strategy
            )
            ...
            run_summary(
                ...
                preproc,
                mask,
                (path_plot, path_none, path_vector, path_qc),
            )
